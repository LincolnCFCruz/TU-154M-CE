...
defineProperty("vbe_alt_left", globalPropertyf("tu-154/gauges/alt/vbe_alt_left"))  -- indicated altitude in meters
defineProperty("vbe_alt_right", globalPropertyf("tu-154/gauges/alt/vbe_alt_right"))  -- indicated altitude in meters
...
defineProperty("mode_set", globalPropertyi("tu-154/taws/mode_set")) -- screen operating mode. 0 - off, 1 - terrain map, 2 - side view, 3 - clock, 4 - power-up sequence, 5 - test

defineProperty("egpws_alarm_1", globalPropertyi("tu-154/switchers/ovhd/egpws_alarm_1")) -- SRPBZ (GPWS) warning, general
defineProperty("egpws_alarm_2", globalPropertyi("tu-154/switchers/ovhd/egpws_alarm_2")) -- SRPBZ flap annunciation
	...
	--	print(mode7_counter, "  ", mode7_left_done, "  ", mode7_center_done, "  ", mode7_right_done)
		...
		if mode7_counter < 0.3 and mode7_counter > 0.25 and not mode7_left_done and mode_7_active then -- left vector
			...
			for row = 1, rows, 1 do
				...
				local lat, lon, alt
				if locationX then lat, lon, alt = localToWorld(locationX, locationY, locationZ) end -- we need alt of prob from here. the probe returns nils outside loaded scenery
				
				local gnd_alt = -100000 -- no terrain data: no threat, so this probe simply does not trigger
				if alt then gnd_alt = alt - acf_alt end
