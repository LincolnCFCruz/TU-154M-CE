...
-- images
defineProperty("wc_screen_img", loadImage("taws_welcome.png", 0, 254, 1000, 770))
...
0:01 - start. yellow screen - 1
0:23 - blue screen and the counter 015 - 2
0:28 - the counter freezes at 010 - 2
0:34 - black screen - 3
0:39 - greeting screen - 4
0:48 - grey screen and the "NET RP" caption on a black background - 5
1:01 - operating mode. 20 km scale. - 6
...
function update()
	...
	if not screen_work then 
		...
		elseif time_counter >= 1  and time_counter < 23 then
			sequence_phase = 1 -- start. yellow screen
		elseif time_counter < 34 then
			sequence_phase = 2 -- blue screen and the counter 015
		elseif time_counter < 39 then
			sequence_phase = 3 -- black screen
		elseif time_counter < 48 then
			sequence_phase = 4 -- greeting screen
		elseif time_counter < 61 then
			sequence_phase = 5 -- grey screen and the "NET RP" caption on a black background
		elseif time_counter >= 61 and time_counter < 62 then
			sequence_phase = 6 -- operating mode. 20 km scale.
			set(mode_set, 1)
			set(distance_set, 1)
		end
