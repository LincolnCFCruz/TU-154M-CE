...
defineProperty("var_on_1", globalPropertyi("tu-154/switchers/ovhd/var_left"))  -- overhead. var
defineProperty("var_on_2", globalPropertyi("tu-154/switchers/ovhd/var_right"))  -- overhead. var
...
-- X-Plane publishes 19 multiplayer slots (plane1..plane19); sim/multiplayer/position/
-- plane20_* has never existed, in XP12 or before. Slot 20 of the tables below stays
-- {0, 0, 0}, which the "show" test in coor_calc() already reads as an empty slot.
...
if MASTER then
	...
	-- results
	set(mode_set, tcas_mode) -- TCAS mode. -1 = test, 0 - stby, 1 = alt off, 2 = alt on, 3 = TA, 4 = TARA	4
...
local function tcas_ra_calc()
	...
	if tcas_mode == 4 then
		...
		if target_1_alt_now and ra_counter > 3 then -- only one target
			...
			-- compute two variants of the vertical speed change and what the altitude difference would be. take the decision with the larger difference
			-- assumed vertical speeds of 5 and 12 m/s.
			-- if the flight is straight and level, assume 5
			-- if descending or climbing - 12 and 0 respectively.
