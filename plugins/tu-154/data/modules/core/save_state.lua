...
local var_table = {}
	...
	var_table["brakeRunLeft"] = get(brake_runtime_left) * 1000
	var_table["brakeRunRight"] = get(brake_runtime_right) * 1000
	...
	-- scaled to match write_file()/read_file(): this table doubles as the
	-- fallback when the key is absent from the state file, so it has to be in
	-- the same units the restore divides back down.
	var_table["hydroQty1"] = get(gs_qty_1) * 100000
	var_table["hydroQty2"] = get(gs_qty_2) * 100000
	var_table["hydroQty3"] = get(gs_qty_3) * 100000
...
local function read_file()
	...
	if savefile then
		...
		if var_table["windowHeatFail2"] then set(window_heat_fail_2, var_table["windowHeatFail2"]) end
		if var_table["windowHeatFail3"] then set(window_heat_fail_3, var_table["windowHeatFail3"]) end
		...
		if var_table["apuFail"] then set(apu_fail, var_table["apuFail"]) end
