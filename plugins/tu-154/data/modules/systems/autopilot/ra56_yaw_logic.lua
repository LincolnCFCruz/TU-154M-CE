...
defineProperty("hod1", globalPropertyf("tu-154/absu/d_ra1_y"))
...
defineProperty("ppn_ra", globalPropertyf("tu-154/t154/ppn13_lamp3"))
...
local d_ra3_p=0
local centr=0
local start1=math.random()*0.1
local start2=math.random()*0.1
local start3=math.random()*0.1
...
function update()
	...
	local ra56_cmd_p=get(absu_cmd_yaw)
	local avt=get(hydro_circuit_auto_man)
	--local absu_work_p=bool2int(get(roll_main_mode)>0)
	local otk1=0
	local otk2=bool2int(get(absu_ra56_yaw_fail)>0)
	local otk3=bool2int(get(absu_ra56_yaw_fail)>1)
	...
	if power27==1 then
		fail1=math.max(p_kolc1*(1-avt),1-get(hydro_ra56_rud_1))
	else
		fail1=1
	end
	
	if power27==1 then
		fail2=math.max(p_kolc2*(1-avt),1-get(hydro_ra56_rud_2))
	else
		fail2=1
	end
	
	if power27==1 then
		fail3=math.max(p_kolc3*(1-avt),1-get(hydro_ra56_rud_3))
	else
		fail3=1
	end
    ...
    if p_kolc1t>0 or ppn_test1 or (get(hydro_ra56_rud_1)==0) then
        p_kolc1=1
    else
        p_kolc1=0
    end
    if p_kolc2t>0 or ppn_test2 or (get(hydro_ra56_rud_2)==0) then
        p_kolc2=1
    else
        p_kolc2=0
    end
    if p_kolc3t>0 or ppn_test3 or (get(hydro_ra56_rud_3)==0) then
        p_kolc3=1
    else
        p_kolc3=0
    end
	...
	if ra56_act_p>yaw_lim then
		ra56_act_p=yaw_lim
	elseif ra56_act_p<-yaw_lim then
		ra56_act_p=-yaw_lim
	end
	
	set(absu_ra1_yaw_fail,p_kolc1)
	set(absu_ra2_yaw_fail,p_kolc2)
	set(absu_ra3_yaw_fail,p_kolc3)
	
	-- set(absu_ra1_yaw_kolc,p_kolc1)
	-- set(absu_ra2_yaw_kolc,p_kolc2)
	-- set(absu_ra3_yaw_kolc,p_kolc3)
	
	set(absu_contr_yaw,ra56_act_p)
	set(hod1,d_ra1_p)
	set(hod2,d_ra2_p)
	set(hod3,d_ra3_p)
