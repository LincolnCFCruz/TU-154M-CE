...
defineProperty("compas_knob", globalPropertyf("tu-154/gauges/misc/compas_knob")) -- heading corrector knob


defineProperty("diss_groundspeed", globalPropertyf("tu-154/nvu/diss_groundspeed")) -- ground speed from the DISS - km/h
...
defineProperty("nvu_fail_signal", globalPropertyi("tu-154/nvu/nvu_fail")) -- failure or not enough systems for the NVU
...
if MASTER then	
	...
	if mode == 2 then -- normal mode
		if active == 1 then
			...
			--[[
			S_pr = SC1 cos Delta ZPU - ZC1 sin Delta ZPU;
            Z_pr = SC1 sin Delta ZPU + ZC1 cos Delta ZPU.
	...
	elseif mode == 3 then -- correction mode
		...
		if active == 1 then
			...
			-- Delta S = S_sm (RSBN) - S_sm (NVU);
            -- Delta Z = Z_sm (RSBN) - Z_sm (NVU).
