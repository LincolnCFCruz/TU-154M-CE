...
defineProperty("msrp_date_ten", globalPropertyf("tu-154/switchers/eng/msrp_date_ten")) -- MSRP date, day tens
...
defineProperty("mgv_roll", globalPropertyf("tu-154/gyro/mgv_contr_roll")) -- AGR roll, + right
...
defineProperty("wind_speed_kt", globalPropertyf("sim/weather/aircraft/wind_now_speed_msc"))  -- XP12: m/s (renamed for code compatibility)
...
-- pluginDataDir.."/output/black_box" is the An-24RV-CE convention (output/ is
-- gitignored). Decode them with the tool in _extras/third_party/BBoxDecoder.zip.
...
function write_file() -- write parameters to file
	...
	if savefile then
		...
		savefile:write("9\t") -- recorded data
			...
			local baro_alt = get(msl_alt) + (29.92 - msl_press_inhg) * 1000 * 0.3048  -- calculate barometric altitude in meters
			...
			savefile:write(math.floor(get(wind_speed_kt)*3.6*100)*0.01, "\t")  -- XP12: m/s * 3.6 = km/h
