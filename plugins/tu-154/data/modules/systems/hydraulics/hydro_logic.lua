...
defineProperty("bak_qty_1", globalPropertyf("tu-154/hydro/gs_bak_qty_1")) -- oil remaining in the tank
defineProperty("bak_qty_2", globalPropertyf("tu-154/hydro/gs_bak_qty_2")) -- oil remaining in the tank
defineProperty("bak_qty_3", globalPropertyf("tu-154/hydro/gs_bak_qty_3")) -- oil remaining in the tank
...
defineProperty("brake_emerg_L", globalPropertyf("tu-154/controlls/brake_emerg_L")) -- emergency brake
defineProperty("brake_emerg_R", globalPropertyf("tu-154/controlls/brake_emerg_R")) -- emergency brake
...
defineProperty("emerg_gear_ext", globalPropertyi("tu-154/controll/emerg_gear_ext")) -- emergency landing gear extension handle
...
defineProperty("hod3_p", globalPropertyf("tu-154/absu/d_ra3_p"))

defineProperty("hod1_r", globalPropertyf("tu-154/absu/d_ra1_r"))
defineProperty("hod2_r", globalPropertyf("tu-154/absu/d_ra2_r"))
defineProperty("hod3_r", globalPropertyf("tu-154/absu/d_ra3_r"))

defineProperty("hod1_y", globalPropertyf("tu-154/absu/d_ra1_y"))
defineProperty("hod2_y", globalPropertyf("tu-154/absu/d_ra2_y"))
...
landing gear oil consumption: 17 litres for each main strut. 8 litres for the nose.
...
-- 41 93
	...
	if hs2_qty > 0 then 
		local flow = elec_pump_2 * interpolate(electric_pumps_t, acc_2) * passed *1.3 * elec_pump_2_start_timer
		acc_2 = acc_2 + flow
		hs2_qty = hs2_qty - flow
	end
	if hs3_qty > 0 then 
		local flow = elec_pump_3 * interpolate(electric_pumps_t, acc_3) * passed *1.25 * elec_pump_3_start_timer
		acc_3 = acc_3 + flow
		hs3_qty = hs3_qty - flow
	end
	
	-- electric_pumps_t is keyed on the accumulator value (0..4.21), the same key
	-- the flow calls above use. Passing acc * 50 (i.e. pressure, 0..210) ran off
	-- the end of the table, so the load term was always 0.
	local pump2_current = elec_pump_2 * (32 + interpolate(electric_pumps_t, acc_2) * 0.15)
	local pump3_current = elec_pump_3 * (32 + interpolate(electric_pumps_t, acc_3) * 0.15)
	...
	-- to be sampled every frame, not once at module scope.
	local push=get(pushback)
	local nws_feed=math.abs(get(nws)-nws_last)*0.004*(1-push)
	nws_last=get(nws)
	...
	local sbd_brk_inn_feed = math.abs(spd_brk_inn_L_now - spd_brk_inn_L_last + spd_brk_inn_R_now - spd_brk_inn_R_last) * base_feed*2
	
	if (spd_brk_inn_L_now - spd_brk_inn_L_last + spd_brk_inn_R_now - spd_brk_inn_R_last)<0 then
		sbd_brk_inn_feed = math.abs(spd_brk_inn_L_now - spd_brk_inn_L_last + spd_brk_inn_R_now - spd_brk_inn_R_last) * base_feed*1.5
	end
	
	local sbd_brk_mid_feed = math.abs(spd_brk_mid_L_now - spd_brk_mid_L_last + spd_brk_mid_R_now - spd_brk_mid_R_last) * base_feed*5.6*2
	
	if (spd_brk_mid_L_now - spd_brk_mid_L_last + spd_brk_mid_R_now - spd_brk_mid_R_last)<0 then
		sbd_brk_mid_feed = math.abs(spd_brk_mid_L_now - spd_brk_mid_L_last + spd_brk_mid_R_now - spd_brk_mid_R_last) * base_feed*3.9*2
	end
	
	local elevon_feed = math.abs(elevon_L_now - elevon_L_last + elevon_R_now - elevon_R_last) * base_feed*0.87*2
	if (elevon_L_now - elevon_L_last + elevon_R_now - elevon_R_last)<0 then
		elevon_feed = math.abs(elevon_L_now - elevon_L_last + elevon_R_now - elevon_R_last) * base_feed*0.52*2
	end
	...
	local absu_pitch_feed3 = math.abs(get(hod3_p)) * 0.0025
	local absu_roll_feed1 = math.abs(get(hod1_r)) * 0.0025
	local absu_roll_feed2 = math.abs(get(hod2_r)) * 0.0025
	local absu_roll_feed3 = math.abs(get(hod3_r)) * 0.0025
	local absu_yaw_feed1 = math.abs(get(hod1_y)) * 0.0025
